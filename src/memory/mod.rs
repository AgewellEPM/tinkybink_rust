...
impl TinkyMemoryCore {
    ...
    pub fn learn_from_interaction(
        ...
        debug!("🧠 Learning from interaction: '{}' → '{}' (success: {:.2})", 
        ...
        info!("🧠 Learned new interaction pattern. Total memories: {}", 
    ...
    pub fn adapt_responses_to_mood(&self, base_responses: Vec<SuggestionTile>) -> Vec<SuggestionTile> {
        ...
        debug!("🎭 Adapting responses for mood: happiness={:.2}, energy={:.2}, anxiety={:.2}", 
               mood.happiness, mood.energy, mood.anxiety);
        
        base_responses.into_iter().map(|mut tile| {
    ...
    fn categorize_topic(&self, question: &str) -> String {
        ...
        if question_lower.contains("hungry") || question_lower.contains("eat") || question_lower.contains("food") {
            "food".to_string()
        } else if question_lower.contains("feel") || question_lower.contains("mood") || question_lower.contains("emotion") {
            "emotions".to_string()
    ...
    fn calculate_recent_success_rate(&self) -> f32 {
        ...
        let total_success: f32 = recent_interactions
            .iter()
