...
impl LlamaEngine {
    ...
    async fn get_or_download_model() -> Result<String> {
        ...
        warn!("   curl -L https://huggingface.co/TheBloke/TinyLlama-1.1B-Chat-v1.0-GGUF/resolve/main/tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf -o models/tinyllama-1.1b-q4_k_m.gguf");
    ...
    fn generate_pattern_based(&self, input: &str, context: &AiContext) -> Result<Vec<AiResponse>> {
        ...
        let responses: Vec<AiResponse> = suggestions.into_iter()
            .map(|text| {
