...
impl MotorAtencion {
    ...
    fn inicializar_pesos(filas: usize, cols: usize) -> Vec<Vec<f32>> {
        ...
        (0..filas).map(|_| (0..cols).map(|_| rand::random::<f32>() * escala - escala / 2.0).collect()).collect()
    }
    ...
    fn proyectar(&self, x: &[Vec<f32>], pesos: &[Vec<f32>]) -> Vec<Vec<f32>> {
        x.iter()
            .map(|fila| {
                pesos.iter().map(|peso_fila| fila.iter().zip(peso_fila.iter()).map(|(a, b)| a * b).sum()).collect()
            })
            .collect()
    }
...
impl BloqueTransformador {
    ...
    fn agregar_residual(&self, x: &[Vec<f32>], residual: &[Vec<f32>]) -> Vec<Vec<f32>> {
        x.iter()
            .zip(residual.iter())
            .map(|(fila_x, fila_r)| fila_x.iter().zip(fila_r.iter()).map(|(a, b)| a + b).collect())
            .collect()
    }
}
    ...
    pub fn adelante(&self, x: &[Vec<f32>]) -> Vec<Vec<f32>> {
        x.iter()
            .map(|fila| {
                ...
                let oculta: Vec<f32> = self
                    .pesos1
                    .iter()
                    .zip(self.sesgo1.iter())
                    .map(|(peso_fila, sesgo)| {
                        let suma: f32 = fila.iter().zip(peso_fila.iter()).map(|(a, b)| a * b).sum::<f32>() + sesgo;
                        self.gelu(suma)
                    })
                    .collect();
                ...
                self.pesos2
                    ...
                    .map(|(peso_fila, sesgo)| {
                        oculta.iter().zip(peso_fila.iter()).map(|(a, b)| a * b).sum::<f32>() + sesgo
                    })
                    .collect()
            })
            .collect()
...
impl NormalizacionCapa {
    pub fn nueva(dim: usize) -> Self {
        Self { gamma: vec![1.0; dim], beta: vec![0.0; dim], epsilon: 1e-5 }
    }
    ...
    pub fn adelante(&self, x: &[Vec<f32>]) -> Vec<Vec<f32>> {
        x.iter()
            .map(|fila| {
                ...
                let varianza: f32 = fila.iter().map(|v| (v - media).powi(2)).sum::<f32>() / fila.len() as f32;

                fila.iter()
                    .zip(self.gamma.iter())
                    .zip(self.beta.iter())
                    .map(|((v, g), b)| ((v - media) / (varianza + self.epsilon).sqrt()) * g + b)
                    .collect()
            })
            .collect()
    }
...
impl NucleoGPTEspañol {
    pub fn nuevo(config: ConfiguracionGPT) -> Self {
        ...
        Self {
            bloques,
            incrustacion_tokens: Self::inicializar_incrustaciones(config.tamaño_vocabulario, config.dim_oculta),
            incrustacion_posicion: Self::inicializar_incrustaciones(config.longitud_contexto, config.dim_oculta),
    ...
    fn inicializar_incrustaciones(num: usize, dim: usize) -> Vec<Vec<f32>> {
        (0..num).map(|_| (0..dim).map(|_| rand::random::<f32>() * 0.02 - 0.01).collect()).collect()
    }
    ...
    fn aplicar_cabeza_lm(&self, x: &[Vec<f32>]) -> Vec<Vec<f32>> {
        x.iter()
            .map(|fila| {
                self.cabeza_lm
                    .iter()
                    .map(|peso_fila| fila.iter().zip(peso_fila.iter()).map(|(a, b)| a * b).sum())
                    .collect()
            })
            .collect()
    }
    ...
    pub fn actualizar_emocion(&mut self, nueva_emocion: EstadoEmocional) {
        ...
        self.estado_emocional.valencia = self.estado_emocional.valencia * 0.7 + nueva_emocion.valencia * 0.3;
        self.estado_emocional.activacion = self.estado_emocional.activacion * 0.7 + nueva_emocion.activacion * 0.3;
        self.estado_emocional.dominancia = self.estado_emocional.dominancia * 0.7 + nueva_emocion.dominancia * 0.3;
        self.estado_emocional.expresividad =
            self.estado_emocional.expresividad * 0.6 + nueva_emocion.expresividad * 0.4;
        self.estado_emocional.pasion = self.estado_emocional.pasion * 0.6 + nueva_emocion.pasion * 0.4;
    }
}

// Re-exportar para uso público
#[allow(unused_imports)]
pub use self::{ConfiguracionGPT as GPTConfig, EstadoEmocional as EmotionalState, NucleoGPTEspañol as SpanishGPTCore};
