...
impl StorageManager {
    ...
    pub async fn export_conversation_history(&self, state: &SystemState, format: ExportFormat) -> Result<String> {
        ...
        let filename = match format {
            ExportFormat::Json => format!("conversation_history_{}.json", timestamp),
            ExportFormat::Csv => format!("conversation_history_{}.csv", timestamp),
            ExportFormat::Txt => format!("conversation_history_{}.txt", timestamp),
        };
